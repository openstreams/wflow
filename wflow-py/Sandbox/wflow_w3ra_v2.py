...
class WflowModel(DynamicModel):  
  ...
  def initial(self):
   ...
   # Add reading of parameters here
    ...
    self.lambd = self.wf_readmap(os.path.join(self.Dir, "staticmaps/lambda.map"),0.0,fail=True)
...
#            self.RLin = (0.65*(pe/Tkelv)**0.14)*StefBolz*Tkelv**4     # (3.3)
        ...
        Ei = scalar(self.T24>0)*(scalar(Pg<Pwet)*fveg*Pg+scalar(Pg>=Pwet)*(fveg*Pwet+fER*(Pg-Pwet)))
