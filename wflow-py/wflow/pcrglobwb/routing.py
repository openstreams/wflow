...
class Routing(object):
    ...
    def __init__(self,iniItems,initialConditions,lddMap, Dir, cloneMap):
        ...
        if configget(iniItems,"routingOptions","debugWaterBalance","True") == "False":
        ...
        if 'includeWaterBodies' in iniItems._sections['routingOptions']: 
            if configget(iniItems,"routingOptions","includeWaterBodies","True") == "False" or\
               configget(iniItems,"routingOptions","includeWaterBodies","True") == "None":
                self.includeWaterBodies = False

        # local drainage direction:
        self.lddMap = vos.readPCRmapClone(iniItems.get("routingOptions","lddMap"),
        ...
        if "minimumChannelWidth" in iniItems._sections['routingOptions']:
            if configget(iniItems,"routingOptions","minimumChannelWidth","None") != "None":\
        ...
        if "constantChannelWidth" in iniItems._sections['routingOptions']:
            if configget(iniItems,"routingOptions","constantChannelWidth","None") != "None":\
        ...
        if "constantChannelDepth" in iniItems._sections['routingOptions']:
            if configget(iniItems,"routingOptions","constantChannelDepth","None") != "None":\
        ...
        self.WaterBodies = waterBodies.WaterBodies(iniItems,self.landmask,self.inputDir,self.cloneMap,self.tmpDir)

        # crop evaporation coefficient for surface water bodies
        self.no_zero_crop_water_coefficient = True
        if configget(iniItems,"routingOptions","cropCoefficientWaterNC","None") == "None":
            self.no_zero_crop_water_coefficient = False
        else:
            self.fileCropKC = vos.getFullPath(\
                     iniItems.get("routingOptions","cropCoefficientWaterNC"),\
                     self.inputDir)
        ...
        if 'maxiumLengthOfSubTimeStep' in iniItems._sections['routingOptions']:
            maxiumLengthOfSubTimeStep = float(iniItems.get("routingOptions","maxiumLengthOfSubTimeStep"))
        ...
        if 'minCropWaterKC' in iniItems._sections['routingOptions']:
            self.minCropWaterKC = float(iniItems.get("routingOptions","minCropWaterKC"))
        ...
        # flood plain options:
        #################################################################################
        self.floodPlain = configget(iniItems,"routingOptions","dynamicFloodPlain","False") == "True"
        if self.floodPlain:
            ...
            if configget(iniItems,"routingOptions","bankfullCapacity","None") != "None" :
            
                self.usingFixedBankfullCapacity = True
                self.predefinedBankfullCapacity = vos.readPCRmapClone(iniItems.get("routingOptions","bankfullCapacity"),\
                                                                          self.cloneMap, self.tmpDir, self.inputDir)
            else:  
        ...
        if configget(iniItems,"globalOptions","debug_to_version_one","False"): self.zeroFracWatAllAndAlways = True
    ...
    def getICs(self,iniItems,iniConditions = None):
        ...
        if iniConditions == None:
            ...
            self.timestepsToAvgDischarge = vos.readPCRmapClone(iniItems.get("routingOptions","timestepsToAvgDischargeIni") ,self.cloneMap,self.tmpDir,self.inputDir)  
            
            self.channelStorage          = vos.readPCRmapClone(iniItems.get("routingOptions","channelStorageIni")          ,self.cloneMap,self.tmpDir,self.inputDir)
            self.readAvlChannelStorage   = vos.readPCRmapClone(iniItems.get("routingOptions","readAvlChannelStorageIni")   ,self.cloneMap,self.tmpDir,self.inputDir) 
            self.avgDischarge            = vos.readPCRmapClone(iniItems.get("routingOptions","avgDischargeLongIni")        ,self.cloneMap,self.tmpDir,self.inputDir) 
            self.m2tDischarge            = vos.readPCRmapClone(iniItems.get("routingOptions","m2tDischargeLongIni")        ,self.cloneMap,self.tmpDir,self.inputDir) 
            self.avgBaseflow             = vos.readPCRmapClone(iniItems.get("routingOptions","avgBaseflowLongIni")        ,self.cloneMap,self.tmpDir,self.inputDir) 
            self.riverbedExchange        = vos.readPCRmapClone(iniItems.get("routingOptions","riverbedExchangeIni")        ,self.cloneMap,self.tmpDir,self.inputDir) 
            
            # New initial condition variable introduced in the version 2.0.2: avgDischargeShort 
            self.avgDischargeShort       = vos.readPCRmapClone(iniItems.get("routingOptions","avgDischargeShortIni")       ,self.cloneMap,self.tmpDir,self.inputDir) 

            # Initial conditions needed for kinematic wave methods
            self.subDischarge            = vos.readPCRmapClone(configget(iniItems,"routingOptions","subDischargeIni","0.0"),self.cloneMap,self.tmpDir,self.inputDir)  
        ...
        if iniConditions == None:
            # read initial conditions from pcraster maps listed in the ini file (for the first time step of the model; when the model just starts)
            self.avgInflow  = vos.readPCRmapClone(configget(iniItems,"routingOptions","avgLakeReservoirInflowShortIni","0,0"),self.cloneMap,self.tmpDir,self.inputDir)
            self.avgOutflow = vos.readPCRmapClone(configget(iniItems,"routingOptions","avgLakeReservoirOutflowLongIni","0.0"),self.cloneMap,self.tmpDir,self.inputDir)
            if configget(iniItems,"routingOptions","waterBodyStorageIni","None") != "None":
                self.waterBodyStorage = vos.readPCRmapClone(iniItems.get("routingOptions","waterBodyStorageIni"), self.cloneMap,self.tmpDir,self.inputDir)
    ...
    def getElevationProfile(self, iniItems):
        ...
        if relativeElevationFileNC == None: 
            ...
            relZFileName = vos.getFullPath(iniItems.get("routingOptions","relativeElevationFiles"),\
            ...
            areaFractions = map(float, iniItems.get("routingOptions","relativeElevationLevels").split(','))
