...
def removeshp(shapein,directory):
    ...
    else: shapein = directory+shapein
    if os.path.exists(directory+shapein):
...
for o, a in opts:       
    if o == '-d': dem_in = a
    if o == '-l': lineshp = a
    if o == '-F': scalefactor = a
    if o == '-p': pointshp = a
    if o == '-R': rivshp = a
    if o == '-C': catchshp = a
    if o == '-O': minorder = int(a)
    if o == '-S': skipldd = True
    if o == '-s': snapgaugestoriver = True
    if o == '-B': burnvalue = float(a)
    if o == '-K': keepall = True
    if o == '-I': lddin = True
...
if not skipldd:
    ...
    if not scalefactor == None:
        ...
        call(('gdalwarp','-overwrite','-s_srs',EPSG,'-t_srs',EPSG,'-tr',str(cellsizescaled),str(-cellsizescaled), '-dstnodata', str(-9999),'-r','cubic',dem_in, dem_scaled))
        dem_in = dem_scaled
    call(('gdal_translate','-of','PCRaster','-a_srs',EPSG,'-ot','Float32',dem_in,dem_map))
    ...
    if not lineshp == None:
        ...
        call(('gdal_translate','-of','GTiff','-a_srs',EPSG,'-ot','Float32',clone_map,line_tif))
        call(('gdal_rasterize','-burn','1','-l',file_att,lineshp,line_tif))
        call(('gdal_translate','-of','PCRaster','-a_srs',EPSG,'-ot','Float32',line_tif,line_map))
