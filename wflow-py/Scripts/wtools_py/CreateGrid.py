...
def main():
    ...
    if not inputfile == None:
        ...
        if fileext == '.shp':
            ...
            extent_in = [geotransform[0], geotransform[3] - nrows * raster_cellsize,
                         geotransform[0] + ncols * raster_cellsize, geotransform[3]]
            ...
            if not spatialref == None:
                ...
                if 'UTM zone' in spatialref:
                    Projection = True
                    zone = spatialref[spatialref.find(
                        'UTM zone') + 9:spatialref.find('UTM zone') + 11]
                    hemisphere = spatialref[spatialref.find(
                        'UTM zone') + 11:spatialref.find('UTM zone') + 12]
                    geodatum = 'UTM' + zone + hemisphere
    ...
    call(('mapattr', '-s', '-R', str(rows), '-C', str(columns), '-x',
          str(xorg), '-y', str(yorg), '-l', cellsize, 'mask\mask.map'))
    ...
    if Projection:
        SHP_srs_LYR = SHP_srs_out.CreateLayer(
            SHP_srs_ATT, srs, geom_type=ogr.wkbPolygon)
    else:
        SHP_srs_LYR = SHP_srs_out.CreateLayer(
            SHP_srs_ATT, geom_type=ogr.wkbPolygon)
    ...
    ring.AddPoint(xorg + columns * float(cellsize),
                  yorg - rows * float(cellsize))
