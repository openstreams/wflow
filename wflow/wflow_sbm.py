...
def infiltration(
    ...
    if (MaxInfiltPath + MaxInfiltSoil) > 0.0:
        InfiltSoil = MaxInfiltSoil * min(
            1.0, max(0.0, UStoreCapacity) / (MaxInfiltPath + MaxInfiltSoil)
        )
        InfiltPath = MaxInfiltPath * min(
            1.0, max(0.0, UStoreCapacity) / (MaxInfiltPath + MaxInfiltSoil)
        )
    else:
        InfiltSoil = 0.0
        ...
        if TransferMethod == 1 and shape_layer == 1:
            ...
            if Sd <= 0.00001:
                ...
                st = (
                    ...
                    * KsatVer
                    * np.exp(-f * z[m])
                    * (min(UStoreLayerDepth[m], L[m] * (thetaS - thetaR)) / Sd)
                )
                ast = min(st, UStoreLayerDepth[m])
                UStoreLayerDepth[m] = UStoreLayerDepth[m] - ast
        else:
            L_sat = L[m] * (thetaS - thetaR)
            UStoreLayerDepth[m], ast = unsatzone_flow_layer(
                UStoreLayerDepth[m], KsatVerFrac[m], KsatVer, f, z[m], L_sat, c[m]
            )
    else:
        ast = 0.0
...
def sbm_cell(
    ...
    slope_ = np.concatenate(
        (static["landSlope"], np.array([0], dtype=static["landSlope"].dtype))
    )
    ...
    for i in range(len(nodes)):
        for j in range(len(nodes[i])):
            ...
            if len(n) > 1:
                L = np.concatenate(
                    (
                        layer["UStoreLayerThickness"][n[0:-1], idx],
                        np.array([dyn["zi"][idx] - sumlayer_0[n[-1]]]),
                    )
                ).astype(np.float64)
            else:
            ...
            UStoreCapacity = (
                static["SoilWaterCapacity"][idx]
                - dyn["SatWaterDepth"][idx]
                - layer["UStoreLayerDepth"][n, idx].sum()
            )
            ...
            (
                ...
                dyn["AvailableForInfiltration"][idx],
                static["PathFrac"][idx],
                static["cf_soil"][idx],
                dyn["TSoil"][idx],
                static["InfiltCapSoil"][idx],
                static["InfiltCapPath"][idx],
                UStoreCapacity,
                modelSnow,
                soilInfReduction,
            )
            ...
            ast, layer["UStoreLayerDepth"][:, idx] = unsatzone_flow(
                layer["UStoreLayerDepth"][:, idx],
                InfiltSoilPath,
                L,
                z,
                layer["KsatVerFrac"][:, idx],
                layer["c"][:, idx],
                static["KsatVer"][idx],
                static["f"][idx],
                static["thetaS"][idx],
            ...
            for k in range(len(L)):
                ...
                if k == 0:
                    ...
                    SaturationDeficit = (
                        static["SoilWaterCapacity"][idx] - dyn["SatWaterDepth"][idx]
                    )
                    ...
                    if shape_layer[0] == 1:
                        soilevapunsat = dyn["PotSoilEvap"][idx] * min(
                            1.0, SaturationDeficit / static["SoilWaterCapacity"][idx]
                        )
                    else:
                    ...
                    if shape_layer[0] == 1:
                        ...
                        if len(L) == 0 or len(L) == 1:
                            soilevapsat = dyn["PotSoilEvap"][idx] * min(
                                1.0,
                                (layer["UStoreLayerThickness"][k, idx] - dyn["zi"][idx])
                                / layer["UStoreLayerThickness"][k, idx],
                            )
                            soilevapsat = min(
                                soilevapsat,
                    ...
                    wetroots = _sCurve(
                        dyn["zi"][idx],
                        a=static["ActRootingDepth"][idx],
                        c=static["rootdistpar"][idx],
                    )
                    dyn["ActEvapSat"][idx] = min(
                        dyn["PotTrans"][idx] * wetroots, dyn["SatWaterDepth"][idx]
                    )
                    dyn["SatWaterDepth"][idx] = (
                        dyn["SatWaterDepth"][idx] - dyn["ActEvapSat"][idx]
                    )
                    ...
                    (
                        layer["UStoreLayerDepth"][k, idx],
                        dyn["ActEvapUStore"][idx],
                        RestPotTrans,
                    ) = actTransp_unsat_SBM(
                        static["ActRootingDepth"][idx],
                        layer["UStoreLayerDepth"][k, idx],
                        sumlayer_0[k],
                        RestPotTrans,
                        dyn["ActEvapUStore"][idx],
            ...
            for k in range(L.size - 1, -1, -1):
                du = max(
                    0,
                    layer["UStoreLayerDepth"][k, idx]
                    - L[k] * (static["thetaS"][idx] - static["thetaR"][idx]),
                )
                layer["UStoreLayerDepth"][k, idx] = (
                    layer["UStoreLayerDepth"][k, idx] - du
                )
                if k > 0:
                    layer["UStoreLayerDepth"][k - 1, idx] = (
                        layer["UStoreLayerDepth"][k - 1, idx] + du
                    )

            Ksat = (
                layer["KsatVerFrac"][len(L) - 1, idx]
                * static["KsatVer"][idx]
            ...
            for k in range(L.size - 1, -1, -1):
                ...
                layer["UStoreLayerDepth"][k, idx] = (
                    layer["UStoreLayerDepth"][k, idx] + toadd
                )
            ...
            DeepKsat = static["KsatVer"][idx] * np.exp(
                -static["f"][idx] * static["SoilThickness"][idx]
            )
            DeepTransfer = min(dyn["SatWaterDepth"][idx], DeepKsat)
            dyn["ActLeakage"][idx] = max(
                0.0, min(static["MaxLeakage"][idx], DeepTransfer)
            )
            ...
            ssf_new[idx], dyn["zi"][idx], ExfiltSatWater = kinematic_wave_ssf(
                ssf_in,
                dyn["ssf"][idx],
                dyn["zi"][idx],
                r,
                static["KsatHorFrac"][idx],
                static["KsatVer"][idx],
                static["landSlope"][idx],
                static["neff"][idx],
                static["f"][idx],
                static["SoilThickness"][idx],
                deltaT,
                static["DL"][idx] * 1000,
                static["DW"][idx] * 1000,
                static["ssfmax"][idx],
            ...
            dyn["SatWaterDepth"][idx] = (
                static["SoilThickness"][idx] - dyn["zi"][idx]
            ) * (static["thetaS"][idx] - static["thetaR"][idx])
            ...
            if len(n_new) > 1:
                L_new = np.concatenate(
                    (
                        layer["UStoreLayerThickness"][n_new[0:-1], idx],
                        np.array([dyn["zi"][idx] - sumlayer_0[n_new[-1]]]),
                    )
                ).astype(np.float64)
            else:
            ...
            for k in range(L.size - 1, -1, -1):
                if (np.where(n_new == k))[0].size > 0:
                    ExfiltFromUstore = max(
                        0,
                        layer["UStoreLayerDepth"][k, idx]
                        - L_new[k] * (static["thetaS"][idx] - static["thetaR"][idx]),
                    )
                else:
                    ExfiltFromUstore = layer["UStoreLayerDepth"][k, idx]
                layer["UStoreLayerDepth"][k, idx] = (
                    layer["UStoreLayerDepth"][k, idx] - ExfiltFromUstore
                )
                if k > 0:
                    layer["UStoreLayerDepth"][k - 1, idx] = (
                        layer["UStoreLayerDepth"][k - 1, idx] + ExfiltFromUstore
                    )
            ...
            if (InfiltSoil + InfiltPath) > 0.0:
                dyn["ActInfiltSoil"][idx] = InfiltSoil - du * InfiltSoil / (
                    InfiltPath + InfiltSoil
                )
                dyn["ActInfiltPath"][idx] = InfiltPath - du * InfiltPath / (
                    InfiltPath + InfiltSoil
                )
            else:
            ...
            if nrpaddyirri > 0:
                if static["h_p"][idx] > 0:
                    ponding_add = min(
                        dyn["ExfiltWater"][idx]
                        + dyn["ExcessWater"][idx]
                        + dyn["InfiltExcess"][idx],
                        static["h_p"][idx] - dyn["PondingDepth"][idx],
                    )
            ...
            dyn["InwaterO"][idx] = (
                max(
                    ...
                    + dyn["ExcessWater"][idx]
                    + dyn["InfiltExcess"][idx]
                    + dyn["RunoffLandCells"][idx]
                    - dyn["ActEvapOpenWaterLand"][idx]
                    - ponding_add,
                    0.0,
                )
                * (static["xl"][idx] * static["yl"][idx])
                * 0.001
                / timestepsecs
            )
            ...
            for k in range(layer["UStoreLayerThickness"][:, idx].size):
                if (np.where(n_new == k))[0].size > 0:
                    if layer["UStoreLayerThickness"][k, idx] > 0:
                        layer["vwc"][k, idx] = (
                            layer["UStoreLayerDepth"][k, idx]
                            + (layer["UStoreLayerThickness"][k, idx] - L_new[k])
                            * (static["thetaS"][idx] - static["thetaR"][idx])
                        ) / layer["UStoreLayerThickness"][k, idx] + static["thetaR"][
                            idx
                        ]
                else:
                    layer["vwc"][k, idx] = static["thetaS"][idx]

                layer["vwc_perc"][k, idx] = (
                    layer["vwc"][k, idx] / static["thetaS"][idx]
                ) * 100.0
            ...
            for k in range(L_new.size):
                if L_new[k] > 0:
                    rootStore_unsat = (
                        rootStore_unsat
                        + (
                            max(0.0, static["ActRootingDepth"][idx] - sumlayer_0[k])
                            / L_new[k]
                        )
                        * layer["UStoreLayerDepth"][k, idx]
                    )
    ...
    acc_flow = np.concatenate(
        (acc_flow, np.array([0], dtype=dyn["LandRunoffsub"].dtype))
    )
    ...
    for v in range(0, it_kinL):
        ...
        qo_new = np.zeros(dyn["LandRunoffsub"].size, dtype=dyn["LandRunoffsub"].dtype)
        qo_new = np.concatenate(
            (qo_new, np.array([0], dtype=dyn["LandRunoffsub"].dtype))
        )
            ...
            for j in range(len(nodes[i])):
                ...
                if static["River"][idx]:
                    ...
                    if static["SW"][idx] > 0.0:
                        ...
                        qo_toriver_vol = np.sum(chanperc * qo_new[nbs]) * (
                            timestepsecs / it_kinL
                        )
                    else:
                        qo_in = 0.0
                ...
                qo_new[idx] = kinematic_wave(
                    qo_in,
                    dyn["LandRunoffsub"][idx],
                    q[idx],
                    static["AlphaL"][idx],
                    static["Beta"][idx],
                    timestepsecs / it_kinL,
                ...
                if static["SW"][idx] > 0:
                    dyn["WaterLevelLsub"][idx] = (
                        static["AlphaL"][idx]
                        * np.power(qo_new[idx], static["Beta"][idx])
                    ) / static["SW"][idx]
    ...
    dyn["SoilWatbal"][:] = (
        dyn["ActInfilt"][:]
        - (
            (dyn["SatWaterDepth"][:] + dyn["sumUStoreLayerDepth"][:])
            - (sumUSold[:] + SWDold[:])
        )
        + (dyn["CellInFlow"][:] - ssf_new[:-1])
        / (static["DW"][:] * static["DL"][:] * 1000 * 1000)
        - dyn["ExfiltWater"][:]
        - dyn["soilevap"][:]
        - dyn["ActEvapUStore"][:]
        - dyn["ActEvapSat"][:]
...
class WflowModel(pcraster.framework.DynamicModel):
    ...
    def initial(self):
        ...
        *Snow and frozen soil modelling parameters*
        ...
        self.kinwaveRiverTstep = int(
            configget(self.config, "model", "kinwaveRiverTstep", "0")
        )
        self.kinwaveLandTstep = int(
            configget(self.config, "model", "kinwaveLandTstep", "0")
        )
        if self.kinwaveIters == 1:
            self.logger.info("Using sub timestep for kinematic wave (iterate)")
            if self.kinwaveRiverTstep > 0:
                self.logger.info(
                    "Using a fixed timestep (seconds) for kinematic wave river flow: "
                    + str(self.kinwaveRiverTstep)
                )
            if self.kinwaveLandTstep > 0:
                self.logger.info(
                    "Using a fixed timestep (seconds) for kinematic wave overland flow: "
                    + str(self.kinwaveLandTstep)
                )
        ...
        self.landSlope = pcr.max(
            0.00001,
            self.wf_readmap(os.path.join(self.Dir, "staticmaps/Slope.map"), self.Slope),
        )
        ...
        self.riverSlope = pcr.max(
            0.00001,
            self.wf_readmap(
                os.path.join(self.Dir, "staticmaps/RiverSlope.map"), self.riverSlope
            ),
        )
        ...
        W = (
            ...
            * (
                pcr.max(
                    0.0001, pcr.windowaverage(self.riverSlope, pcr.celllength() * 4.0)
                )
            )
            ** (-0.1875)
            * self.NRiver ** (0.375)
        ...
        self.SW = pcr.ifthenelse(
            self.River, pcr.max(self.DW - self.RiverWidth, 0), self.DW
        )

        # term for Alpha (Land)
        self.AlpTermL = pow((self.N / (pcr.sqrt(self.landSlope))), self.Beta)
        self.AlphaL = self.AlpTermL * pow(self.SW, self.AlpPow)

        # use fixed alpha value based on 0.5 * bankfull_depth
        self.AlphaR = self.AlpTermR * pow(self.Bw + self.BankfullDepth, self.AlpPow)
        ...
        for n in range(self.maxLayers):
            ...
            self.layer["KsatVerFrac"][n] = pcr.pcr2numpy(
                self.KsatVerFrac[n], self.mv
            ).ravel()
        ...
        self.static["SoilThickness"] = pcr.pcr2numpy(
            self.SoilThickness, self.mv
        ).ravel()
        self.static["InfiltCapSoil"] = pcr.pcr2numpy(
            self.InfiltCapSoil, self.mv
        ).ravel()
        self.static["InfiltCapPath"] = pcr.pcr2numpy(
            self.InfiltCapPath, self.mv
        ).ravel()
        ...
        self.static["SoilWaterCapacity"] = pcr.pcr2numpy(
            self.SoilWaterCapacity, self.mv
        ).ravel()
        self.static["CanopyGapFraction"] = pcr.pcr2numpy(
            self.CanopyGapFraction, self.mv
        ).ravel()
        ...
        self.static["AirEntryPressure"] = pcr.pcr2numpy(
            self.AirEntryPressure, self.mv
        ).ravel()
        ...
        self.static["AlphaR"] = pcr.pcr2numpy(self.AlphaR, self.mv).ravel()
        self.static["AlphaL"] = pcr.pcr2numpy(self.AlphaL, self.mv).ravel()
        self.static["ssfmax"] = (
            (
                self.static["KsatHorFrac"]
                * self.static["KsatVer"]
                * self.static["landSlope"]
            )
            / self.static["f"]
            * (np.exp(0) - np.exp(-self.static["f"] * self.static["SoilThickness"]))
        )
        ...
        for n in np.arange(0, self.maxLayers):
            ...
            if self.nrLayers > 1 and n < self.nrLayers:
                UstoreThick_temp = float(UStoreLayerThickness.split(",")[n]) + np_zeros
                UstoreThick = np.minimum(
                    UstoreThick_temp,
                    np.maximum(self.static["SoilThickness"] - np_sumL, 0.0),
                )
            else:
                UstoreThick_temp = np.maximum(
                    self.static["SoilThickness"] - np_sumL, 0.0
                )
                UstoreThick = np.maximum(self.static["SoilThickness"] - np_sumL, 0.0)

            np_SumThickness = UstoreThick_temp + np_SumThickness

            nrLayersMap = np.where(
                self.static["SoilThickness"] - np_sumL > 0.0,
                nrLayersMap + 1.0,
                nrLayersMap,
            )
    ...
    def resume(self):
        ...
        if self.reinit == 1:
            ...
            self.zi = pcr.max(
                0.0,
                self.SoilThickness - self.SatWaterDepth / (self.thetaS - self.thetaR),
            )

            self.SubsurfaceFlow = (
                ((self.KsatHorFrac * self.KsatVer * self.landSlope) / pcr.abs(self.f))
                * (
                    pcr.exp(-pcr.abs(self.f) * (self.zi))
                    - pcr.exp(-pcr.abs(self.f) * (self.SoilThickness))
                )
                * self.DW
                * 1000
            )
    ...
    def dynamic(self):
        ...
        if self.modelSnow:
            ...
            if hasattr(self, "GlacierFrac"):
                ...
                self.RainFallPlusMelt = self.RainFallPlusMelt + pcr.cover(
                    self.GlacierMelt, 0.0
                )
        else:
        ...
        self.AvailableForInfiltration = self.RainFallPlusMelt + self.IRSupplymm
        ...
        self.RunoffRiverCells = (
            pcr.min(1.0, self.RiverFrac) * self.AvailableForInfiltration
        )
        self.RunoffLandCells = (
            pcr.min(1.0, self.WaterFrac) * self.AvailableForInfiltration
        )

        self.dyn["RunoffLandCells"] = pcr.pcr2numpy(
            self.RunoffLandCells, self.mv
        ).ravel()
        ...
        self.static["ActRootingDepth"] = pcr.pcr2numpy(
            self.ActRootingDepth, self.mv
        ).ravel()
        ...
        self.dyn["ActEvapOpenWaterLand"] = pcr.pcr2numpy(
            self.ActEvapOpenWaterLand, self.mv
        ).ravel()
        self.dyn["ActEvapOpenWaterRiver"] = pcr.pcr2numpy(
            self.ActEvapOpenWaterRiver, self.mv
        ).ravel()

        self.RestEvap = (
            self.PotTransSoil - self.ActEvapOpenWaterRiver - self.ActEvapOpenWaterLand
        )
        ...
        self.dyn["AvailableForInfiltration"] = pcr.pcr2numpy(
            self.AvailableForInfiltration, self.mv
        ).ravel()
        ...
        for i in range(self.maxLayers):
            self.layer["UStoreLayerDepth"][i] = pcr.pcr2numpy(
                self.UStoreLayerDepth[i], self.mv
            ).ravel()

        self.dyn["LandRunoffsub"] = pcr.pcr2numpy(self.LandRunoffsub, self.mv).ravel()
        self.dyn["sumUStoreLayerDepth"] = pcr.pcr2numpy(
            self.UStoreDepth, self.mv
        ).ravel()

        it_kinL = 1
        if self.kinwaveIters == 1:
            if self.kinwaveLandTstep == 0:
                it_kinL = estimate_iterations_kin_wave(
                    self.LandRunoffsub,
                    self.Beta,
                    self.AlphaL,
                    self.timestepsecs,
        ...
        ssf, qav, wl_av, self.dyn, self.layer = sbm_cell(
            self.nodes,
            self.nodes_up,
            self.np_ldd.ravel(),
            self.layer,
        ...
        self.SubsurfaceFlow = pcr.numpy2pcr(
            pcr.Scalar, ssf.reshape(self.shape), self.mv
        )
        self.LandRunoffsub = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["LandRunoffsub"].reshape(self.shape)), self.mv
        )
        self.WaterLevelLsub = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["WaterLevelLsub"].reshape(self.shape)), self.mv
        )
        ...
        self.zi = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["zi"].reshape(self.shape)), self.mv
        )
        self.SatWaterDepth = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["SatWaterDepth"].reshape(self.shape)), self.mv
        )
        self.UStoreDepth = pcr.numpy2pcr(
            pcr.Scalar,
            np.copy(self.dyn["sumUStoreLayerDepth"].reshape(self.shape)),
            self.mv,
        )
        self.CapFlux = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["CapFlux"].reshape(self.shape)), self.mv
        )
        self.Transfer = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["Transfer"].reshape(self.shape)), self.mv
        )

        for i in range(self.maxLayers):
            self.UStoreLayerDepth[i] = pcr.numpy2pcr(
                pcr.Scalar,
                np.copy(self.layer["UStoreLayerDepth"][i].reshape(self.shape)),
                self.mv,
            )

        if self.nrpaddyirri > 0:
            self.PondingDepth = pcr.numpy2pcr(
                pcr.Scalar,
                np.copy(self.dyn["PondingDepth"].reshape(self.shape)),
                self.mv,
            )

        # Determine transpiration
        self.ActEvapUStore = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ActEvapUStore"].reshape(self.shape)), self.mv
        )
        self.ActEvapSat = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ActEvapSat"].reshape(self.shape)), self.mv
        )
        self.Transpiration = self.ActEvapUStore + self.ActEvapSat
        self.soilevapsat = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["soilevapsat"].reshape(self.shape)), self.mv
        )
        self.soilevap = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["soilevap"].reshape(self.shape)), self.mv
        )
        ...
        self.Recharge = (
            self.Transfer - self.CapFlux - self.ActEvapSat - self.soilevapsat
        )
        ...
        if self.nrpaddyirri > 0:
            ...
            sqmarea = pcr.areatotal(self.xl * self.yl, self.IrrigationPaddyAreas)
        ...
        self.qo_toriver = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["qo_toriver"].reshape(self.shape)), self.mv
        )
        self.ssf_toriver = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ssf_toriver"].reshape(self.shape)), self.mv
        )
        self.Inwater = (
            self.InwaterMM * self.ToCubic + self.qo_toriver + self.ssf_toriver
        )  # m3/s

        self.ExfiltWater = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ExfiltWater"].reshape(self.shape)), self.mv
        )
        self.InfiltExcess = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["InfiltExcess"].reshape(self.shape)), self.mv
        )
        self.ExcessWater = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ExcessWater"].reshape(self.shape)), self.mv
        )
        self.ActInfilt = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ActInfilt"].reshape(self.shape)), self.mv
        )
        self.ActLeakage = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ActLeakage"].reshape(self.shape)), self.mv
        )
        self.soilevap = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["soilevap"].reshape(self.shape)), self.mv
        )

        self.SoilInf = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["SoilInf"].reshape(self.shape)), self.mv
        )
        self.PathInf = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["PathInf"].reshape(self.shape)), self.mv
        )
        self.InfiltExcessSoil = pcr.numpy2pcr(
            pcr.Scalar,
            np.copy(self.dyn["ExcessWaterSoil"].reshape(self.shape)),
            self.mv,
        )
        self.InfiltExcessPath = pcr.numpy2pcr(
            pcr.Scalar,
            np.copy(self.dyn["ExcessWaterPath"].reshape(self.shape)),
            self.mv,
        )
        self.ActInfiltSoil = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ActInfiltSoil"].reshape(self.shape)), self.mv
        )
        self.ActInfiltPath = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ActInfiltPath"].reshape(self.shape)), self.mv
        )

        self.ExfiltFromUstore = pcr.numpy2pcr(
            pcr.Scalar,
            np.copy(self.dyn["ExfiltFromUstore"].reshape(self.shape)),
            self.mv,
        )
        self.ExfiltFromSat = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ExfiltSatWater"].reshape(self.shape)), self.mv
        )
        self.SumEvapSat = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["soilevapsat"].reshape(self.shape)), self.mv
        ) + pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ActEvapSat"].reshape(self.shape)), self.mv
        )
        self.SumEvapUstore = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["soilevapunsat"].reshape(self.shape)), self.mv
        ) + pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["ActEvapUStore"].reshape(self.shape)), self.mv
        )
        ...
        self.RiverRunoff = pcr.numpy2pcr(
            pcr.Scalar, Qriver.reshape(self.shape), self.mv
        )
        self.WaterLevelR = pcr.numpy2pcr(
            pcr.Scalar, wl_river_av.reshape(self.shape), self.mv
        )
        self.RiverRunoffsub = pcr.numpy2pcr(
            pcr.Scalar, np.copy(river_runoff_sub).reshape(self.shape), self.mv
        )
        self.WaterLevelRsub = pcr.numpy2pcr(
            ...
            while True:
                ...
                if self.kinwaveIters == 1:
                    if self.kinwaveRiverTstep == 0:
                        ...
                        it_kinR = int(
                            np.ceil(self.timestepsecs / self.kinwaveRiverTstep)
                        )
                ...
                Qriver = acc_flow / self.timestepsecs
                self.RiverRunoff = pcr.numpy2pcr(
                    pcr.Scalar, np.copy(Qriver).reshape(self.shape), self.mv
                )
                self.WaterLevelR = pcr.numpy2pcr(
                    pcr.Scalar, np.copy(wl_river_av).reshape(self.shape), self.mv
                )
                self.RiverRunoffsub = pcr.numpy2pcr(
                    pcr.Scalar, np.copy(river_runoff_sub).reshape(self.shape), self.mv
                )
                self.WaterLevelRsub = pcr.numpy2pcr(
                    pcr.Scalar, np.copy(wl_river).reshape(self.shape), self.mv
                )
                ...
                self.InflowKinWaveCell = pcr.upstream(self.TopoLdd, self.OldRiverRunoff)
                deltasup = float(
        ...
        self.InwaterL = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["InwaterO"].reshape(self.shape)), self.mv
        )
        self.InflowKinWaveCellLand = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["Qo_in"].reshape(self.shape)), self.mv
        )
        ...
        self.RootStore_unsat = pcr.numpy2pcr(
            pcr.Scalar,
            np.copy(self.dyn["RootStore_unsat"].reshape(self.shape)),
            self.mv,
        )
        ...
        for i in range(self.maxLayers):
            self.vwc.append(
                pcr.numpy2pcr(
                    pcr.Scalar,
                    np.copy(self.layer["vwc"][i].reshape(self.shape)),
                    self.mv,
                )
            )
            self.vwc_perc.append(
                pcr.numpy2pcr(
                    pcr.Scalar,
                    np.copy(self.layer["vwc_perc"][i].reshape(self.shape)),
                    self.mv,
                )
            )
        ...
        CellInFlow = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["CellInFlow"].reshape(self.shape)), self.mv
        )
        ...
        self.SoilWatbal = pcr.numpy2pcr(
            pcr.Scalar, np.copy(self.dyn["SoilWatbal"].reshape(self.shape)), self.mv
        )
