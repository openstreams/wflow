...
def dw_mkDelwaqPointers(ldd, amap, gmap, comp, flux, Emission, Aggregation, bd_id):
    ...
    *amap* is used to link boundaries to the segments for delwaq (negative
    ...
    if Aggregation:
        ...
        flowtosc = pcr.ifthenelse(
            pcr.defined(amap) != pcr.defined(flowtosc), amap, flowtosc
        )
    ...
    np_cpid = np.arange(1, len(np_ptid) * len(comp) + 1).reshape(
        len(np_ptid) * len(comp), 1
    )
    np_catchidcp = np.tile(np_catchid[:, 0], len(comp)).reshape(
        len(np_ptid) * len(comp), 1
    )
    ...
    flow_label = (
        np.char.array(comp_label[np_flowtocp != 0])
        + np.char.array(np.repeat("2", NOLF))
        + np.char.array(comp_label[np_flowtocp != 0])
        + np.char.array(np.repeat("_", NOLF))
    ...
    for i in np.arange(0, len(comp)):
        if np.isin(comp.ID[i], compWithLat.values):
            flow_var = np.append(
                flow_var, compWithLat_var[compWithLat.values == comp.ID[i]].values
            )
    ...
    # Update flow labels
    lo_label = comp_label[np_flowtocp != 0].reshape(
        len(comp_label[np_flowtocp != 0]), 1
    )
    lo_label = lo_label[lowerck, 0].reshape(len(lopt), 1)
    flow_label[lowerck] = np.char.array(lo_label) + np.char.array(
        np.repeat("2Outflow", len(lopt)).reshape(len(lopt), 1)
    )
    ...
    if not compflux.empty:
        ...
        for i in np.arange(0, len(compflux)):
            ...
            vertcomp_label_i = np.char.array(
                np.repeat(
                    (compflux.From.iloc[i] + "2" + compflux.To.iloc[i] + "_"),
                    len(comp_flowfrom),
                )
            ) + np.char.array(np_catchid.astype(np.unicode).flatten())
            vertcomp_label_i = vertcomp_label_i.reshape(len(vertcomp_label_i), 1)
            if comps == 1:
                vertcompflux = np.hstack(
                    (comp_flowfrom, comp_flowto, zzerocol, zzerocol)
                )
                vertcomp_label = vertcomp_label_i
            else:
                vertcompflux = np.vstack(
                    (
                        vertcompflux,
                        np.hstack((comp_flowfrom, comp_flowto, zzerocol, zzerocol)),
                    )
                )
    ...
    if not boundflux.empty:
        ...
        for i in np.arange(0, len(boundflux)):
            if bd_id == "segments":
                bounid = (
                    np.arange(start, (len(np_ptid) + start)).reshape((len(np_ptid), 1))
                    * -1.0
                )
            else:
            ...
            if bd_id == "segments":
                bound_label_i = np.char.array(
                    np.repeat(
                        (boundflux.From.iloc[i] + "2" + boundflux.To.iloc[i] + "_"),
                        len(np_ptid),
                    )
                ) + np.char.array(np_catchid.astype(np.unicode).flatten())
            else:
                bound_label_i = np.char.array(
                    np.repeat(
                        (boundflux.From.iloc[i] + "2" + boundflux.To.iloc[i]),
                        len(np_ptid),
                    )
                )
            bound_label_i = bound_label_i.reshape(len(bound_label_i), 1)
            if bouns == 1:
                bound_label = bound_label_i
            else:
                bound_label = np.vstack((bound_label, bound_label_i))
...
def dw_WriteAttributes(fname, comp, cells):
    ...
    for i in np.arange(0, len(comp)):
        print(
            "     %d*%d%d ; %s" % (nb_cells, comp.At2[i], comp.At1[i], comp.ID[i]),
            file=exfile,
        )
...
def dw_WriteBoundlist(fname, flow_labels, pointer_labels, bd_id):
    ...
    boundlist = np.hstack(
        (
            NodeID.reshape(len(NodeID), 1),
            Number.reshape(len(Number), 1),
            boundflow_labels.reshape(len(Number), 1),
        )
    )
...
def dw_mkDelwaqMonitoring(
    mon_points, mon_areas, gmap, amap, comp, cells, segments, seg_labels
):
    """
    ...
    if NOPT > 0:
        outlocname = (
            np.char.array(np.repeat("'", NOPT))
            + np.char.array(np.repeat(ID, NOPT))
            + np.char.array(cell_ID.astype(np.unicode))
            + np.char.array(np.repeat("_", NOPT))
            + np.char.array(comp_list)
            + np.char.array(np.repeat("'", NOPT))
        )
        onecol = np.repeat(1, NOPT).reshape(NOPT, 1)
        balcol = np.repeat("NO_BALANCE", NOPT).reshape(NOPT, 1)
        stations = np.hstack(
            (outlocname.reshape(NOPT, 1), balcol, onecol, point_list.reshape(NOPT, 1))
        )
        stations_balance = np.hstack(
            (outlocname.reshape(NOPT, 1), onecol, point_list.reshape(NOPT, 1))
        )
    else:
        stations = []
        stations_balance = []
...
def dw_WriteMonAreas(fname, NOAR, mon_areas, amap, comp, segments, seg_labels, cells):
    ...
    if mon_areas == "subcatch":
        area_ID = (
            ...
            + np.char.array(np.repeat("Subcatch", NOAR))
            + np.char.array(np.int_(np.unique(np_amap)).astype(np.unicode))
            + np.char.array(np.repeat("'", NOAR))
        )
...
def dw_WriteGeometry(
    fname, cells, amap, reallength, PathFrac, WaterFrac, Aggregation, WriteAscii, NOCP
):
    """
    ...
    fPaved = np.around(
        PathFrac * (np.repeat(1.0, len(PathFrac)) - WaterFrac), decimals=3
    )
    fUnPaved = np.around(
        (np.repeat(1.0, len(PathFrac)) - PathFrac)
        * (np.repeat(1.0, len(PathFrac)) - WaterFrac),
        decimals=3,
    )
    ...
    if Aggregation:
        ...
        for i in range(1, len(cells) + 1):
            ...
            WaterFracsc = np.append(
                WaterFracsc,
                sum(WaterFrac[np_amap == i] * realarea[np_amap == i]) / sc_area,
            )
            fPavedsc = np.append(
                fPavedsc, sum(fPaved[np_amap == i] * realarea[np_amap == i]) / sc_area
            )
            fUnPavedsc = np.append(
                fUnPavedsc,
                sum(fUnPaved[np_amap == i] * realarea[np_amap == i]) / sc_area,
            )
        realarea = realareasc
    ...
    outlocname = (
        np.char.array(np.repeat("; ", len(cells)))
        + np.char.array(np.repeat("Cell", len(cells)))
        + np.char.array((np.int_(cells)).astype(np.unicode))
    )

    if WriteAscii:
        geometry_data = np.hstack(
            (
                realarea.reshape(len(cells), 1),
                fPaved.reshape(len(cells), 1),
                fUnPaved.reshape(len(cells), 1),
                WaterFrac.reshape(len(cells), 1),
...
def dw_WriteFractionParameterIds(fname, boundflow_labels):
    ...
    print(
        "Initial;Fraction of initial water;TRUE;WaterFraction;instantaneous;-;0.001",
        file=exfile,
    )
    for i in boundflow_labels:
        print(
            "%s;Fraction of water from source %s;TRUE;WaterFraction;instantaneous;-;0.001"
            % (i, i),
            file=exfile,
        )
...
def dw_GetGridDimensions(ptid_map):
    ...
    allx = dw_pcrToDataBlock(
        pcr.xcoordinate(pcr.boolean(pcr.cover(zero_map + 1, 1))), zero_map
    )
    i = 0
...
def dw_Write_Times(dwdir, T0, timeSteps, timeStepSec, configfile):
    ...
    while len(toprint) > 0:
        statname = configget(configfile, thissection, "name", "sim")
        ...
        operation = configget(configfile, thissection, "output-operation", "STADSC")
...
def dw_WriteEmiData(
    caseId, outdir, ptid, cells, modelmap, emiData, comp, Aggregation, logger
):
    """
...
def main(argv=None):
    ...
    #    )
    xl, yl, reallength = pcrut.detRealCellLength(amap, sizeinmetres)
    ...
    compFile = (
        caseId
        + "/"
        + configget(config, "inputcsv", "compartments", "/csv/compartments.csv")
    )
    comp = pd.read_csv(compFile, sep=sepcsv, header=0)
    boundFile = (
        caseId
        + "/"
        + configget(config, "inputcsv", "boundaries", "/csv/boundaries.csv")
    )
    ...
    emiFile = (
        caseId + "/" + configget(config, "inputcsv", "emissions", "/csv/emissions.csv")
    )
    ...
    if Aggregation and inputType != "csv":
        logger.error(
            "Aggregation of wflow_sbm results is not implemented with netcdf or PCRaster maps, use csv files instead."
        )
    if inputType == "csv" and not Aggregation:
        logger.warning(
            "CSV input file detected but aggregation option is not set. Make sure CSV inputs are saved for all cells and not aggregated cells."
        )
    if Aggregation and (mon_points == "gauges" or mon_areas == "subcatch"):
        logger.error(
            "Chosen monitoring option(s) are not available with aggregation option"
        )
    ...
    (
        ...
        seg_labels,
        flow_var,
    ) = dw_mkDelwaqPointers(ldd, amap, gmap, comp, flux, Emission, Aggregation, bd_id)
    ...
    if WriteStructure:
        ...
        dw_WriteAttributes(
            dwdir + "/includes_deltashell/B3_attributes.inc", comp, cells
        )
        ...
        boundflow_labels = dw_WriteBoundlist(
            dwdir + "/includes_deltashell/B5_boundlist.inc",
            flow_labels,
            pointer_labels,
            bd_id,
        )
        ...
        if Fraction:
            ...
            dw_WriteInitials(
                dwdir + "/includes_deltashell/B8_initials.inc", boundflow_labels
            )
            dw_Write_Substances(
                dwdir + "/includes_deltashell/B1_sublist.inc", boundflow_labels
            )
            if Fews:
                dw_WriteFractionParameterIds(
                    dwdir + "/includes_fews/fraction_parameterids.csv", boundflow_labels
                )

        # Monitoring files
        logger.info("Writing monitoring files...")
        NOPT, NOAR, stations, stations_balance = dw_mkDelwaqMonitoring(
            mon_points, mon_areas, gmap, amap, comp, cells, segments, seg_labels
        )
        dw_WriteNrMonAreas(
            dwdir + "/includes_deltashell/B2_nrofmon.inc",
            NOPT,
            NOAR,
            mon_points,
            mon_areas,
        ...
        if NOPT != 0:
            ...
            dw_WriteStations(
                dwdir + "/includes_deltashell/B2_stations-balance.inc", stations_balance
            )
        if NOAR != 0:
            dw_WriteMonAreas(
                dwdir + "/includes_deltashell/B2_monareas.inc",
                NOAR,
                mon_areas,
                amap,
                comp,
    ...
    if WriteDynamic:
        ...
        if not Emission:
            logger.info(
                "Writing surface.dat. Nr of points: " + str(np.size(surface_block))
            )
            dw_WriteSegmentOrExchangeData(
            ...
            for i in np.arange(0, len(comp)):
                if comp.At2[i] == 0:
                    ...
                    Manning_block = np.append(
                        Manning_block, dw_pcrToDataBlock(Manning_map, amap, Aggregation)
                    )
                else:
    ...
    if WriteDynamic:
        ...
        if inputType == "netcdf":
            ...
            nc = wf_netcdfio.netcdfinput(netcdffile, logger, var_list)
        else:
            nc = None
        ...
        dw_Write_Times(
            dwdir + "/includes_deltashell/", T0, timeSteps, timestepsecs, config
        )
        ...
        if inputType == "netcdf" or inputType == "map":
            for i in range(
                firstTimeStep, (int(timeSteps) + 1) * int(timestepsecs), timestepsecs
            ):
                ...
                if not Emission:
                    ...
                    for c in range(0, len(comp)):
                        ...
                        if i == firstTimeStep:
                            ...
                            if comp.wflowVar[c] == "UStoreDepth":
                                ...
                                UStoreLayerThickness = configget(
                                    config, "model", "UStoreLayerThickness", "0"
                                )
                                ...
                                for USi in np.arange(0, maxLayers):
                                    volume_map_path = (
                                        caseId
                                        + "/instate/UStoreLayerDepth_"
                                        + str(USi)
                                        + ".map"
                                    )
                                    if os.path.exists(volume_map_path):
                                        volume_map = volume_map + pcr.readmap(
                                            volume_map_path
                                        )

                            # If KinWaveVolume is used instead of WaterLevel:
                            elif (
                                comp.wflowVar[c] == "KinWaveVolumeR+KinWaveVolumeL"
                                or comp.wflowVar[c] == "KinWaveVolumeL+KinWaveVolumeR"
                            ):
                                volume_map_path1 = (
                                    caseId + "/instate/" + "WaterLevelR" + ".map"
                                )
                                volume_map_path2 = (
                                    caseId + "/instate/" + "WaterLevelL" + ".map"
                                )
                                if os.path.exists(volume_map_path):
                                    volume_map1 = pcr.readmap(volume_map_path1)
                                    volume_map2 = pcr.readmap(volume_map_path2)
                                    volume_map = (
                                        volume_map1 + volume_map2
                                    ) * surfaceWater_map
                                else:
                                    volume_map = zeroMap * 0.0
                            # Other compartments
                                ...
                                for wv in range(len(wflowVars)):
                                    volume_map_path = (
                                        caseId + "/instate/" + wflowVars[wv] + ".map"
                                    )
                                    if os.path.exists(volume_map_path):
                                        volume_map = volume_map + pcr.readmap(
                                            volume_map_path
                        ...
                        # Other timesteps
                            ...
                            for wv in range(len(wflowVars)):
                                volume_map = volume_map + read_timestep(
                                    nc, wflowVars[wv], (ts - 1), logger, caseId, runId
                                )
                    ...
                    dw_WriteSegmentOrExchangeData(
                        float(i),
                        dwdir + "/includes_flow/volume.dat",
                        volume_block,
                        1,
                        WriteAscii,
                    )
                ...
                for f in np.arange(0, len(flow_var)):
                    ...
                    if i == (int(timeSteps) + 1) * int(timestepsecs):
                        for v in range(len(wflowVars)):
                            flow_map = flow_map + read_timestep(
                                nc, wflowVars[v], (ts - 1), logger, caseId, runId
                            )
                    else:
                        for v in range(len(wflowVars)):
                            flow_map = flow_map + read_timestep(
                                nc, wflowVars[v], ts, logger, caseId, runId
                            )
                    ...
                    if Emission:
                        ...
                        addtotflow_f = flux.EmPointerHyd[
                            flux.mapstack == flow_var[f]
                        ].values
                        if CalcTotflow and (
                            bool(
                                np.isin(
                                    addtotflow_f[0], np.array(["T", "PT", "HT", "PHT"])
                                )
                            )
                        ):
                            # logger.info("Adding " + str(flow_var[f]) + " to total flow")
                            totalflow_map = totalflow_map + flow_map
                        addflowblock_f = flux.EmPointerHyd[
                            flux.mapstack == flow_var[f]
                        ].values
                        if bool(
                            np.isin(
                                addflowblock_f[0], np.array(["H", "PH", "HT", "PHT"])
                            )
                        ):
                            ...
                            flow_block = np.append(
                                flow_block, dw_pcrToDataBlock(flow_map, amap)
                            )
                    else:
                        flow_block = np.append(
                            flow_block, dw_pcrToDataBlock(flow_map, amap)
                    ...
                    if not Emission:
                        ...
                        if (
                            wflowVar_f[0] == "RiverRunoff+LandRunoff"
                            or wflowVar_f[0] == "LandRunoff+RiverRunoff"
                        ):
                            if i == firstTimeStep:
                                ...
                                level_mapstack = comp.mapstack[
                                    comp.wflowVar == "WaterLevelR+WaterLevelL"
                                ].values
                                if len(level_mapstack) == 0:
                                    level_mapstack = comp.mapstack[
                                        comp.wflowVar == "WaterLevelL+WaterLevelR"
                                    ].values
                                level_map = zeroMap * 0.0
                                wflowVars = level_mapstack[0].split("+")
                                for v in range(len(wflowVars)):
                                    level_map = level_map + read_timestep(
                                        nc,
                                        wflowVars[v],
                                        (ts - 1),
                                        logger,
                            ...
                            area_map = level_map * pcr.readmap(
                                caseId + "/" + runId + "/outsum/Bw.map"
                            )
                        ...
                        # TODO: define real exchange area for soil (bulk density, porosity)
                            ...
                            if i == firstTimeStep:
                                ...
                                level_mapstack = comp.mapstack[
                                    comp.wflowVar == "SatWaterDepth"
                                ].values
                                level_map = read_timestep(
                                    nc,
                                    level_mapstack[0],
                                    (ts - 1),
                                    logger,
                        ...
                        area_block = np.append(
                            area_block, dw_pcrToDataBlock(area_map, amap)
                        )
                        velocity_block = np.append(velocity_block, velocity)

                        # Convert flow from mm to m3/s
                        conv_block = conv_block == 1
                        flow_block[conv_block] = (
                            (flow_block[conv_block] / 1000)
                            * area_block[conv_block]
                            / timestepsecs
                        )
                        ...
                        for c in np.arange(0, len(comp)):
                            if (
                                comp.wflowVar[c] == "LandRunoff+RiverRunoff"
                                or comp.wflowVar[c] == "RiverRunoff+LandRunoff"
                            ):
                                calcv = velocity_block == 1
                                velocity = np.append(
                                    velocity, flow_corr[calcv] / area_corr[calcv]
                                )
                ...
                if Emission:
                    ...
                    if CalcTotflow:
                        ...
                        flow_block = np.append(
                            flow_block, dw_pcrToDataBlock(totalflow_map, amap)
                        )
                    # logger.info("Writing flow.dat. Nr of points: " + str(size(flow_block)))
                    dw_WriteSegmentOrExchangeData(
                        float(i),
                        dwdir + "/includes_flow/hydrology.bin",
                        flow_block,
                        len(comp),
                        WriteAscii,
                    )
                    ...
                    dw_WriteSegmentOrExchangeData(
                        float(i),
                        dwdir + "/includes_flow/velocity.dat",
                        velocity,
                        1,
                        WriteAscii,
                    )
                    ...
                    if Fews:
                        ...
                        dw_WriteSegmentOrExchangeData(
                            i, comroot + ".flo", flow_block, 1, WriteAscii
                        )
                        dw_WriteSegmentOrExchangeData(
        ...
        if inputType == "csv":
            ...
            if not Emission:
                for c in np.arange(0, len(comp)):
                    ...
                    if "0" in volume_csv:
                        del volume_csv["0"]
                    if "# Timestep" in volume_csv:
                        del volume_csv["# Timestep"]

                    # Add potential sum of variables
                    if len(wflowVars) > 1:
                        for v in range(len(wflowVars) - 1):
                            volume_path = (
                                caseId + "/" + runId + "/" + wflowVars[v + 1] + ".csv"
                            )
                            volume_csv2 = pd.read_csv(volume_path, sep=",", header=0)
                            if "0" in volume_csv2:
                                del volume_csv2["0"]
                            if "# Timestep" in volume_csv2:
                                del volume_csv2["# Timestep"]
                    ...
                    # Other compartments
                        ...
                        for v in len(wflowVars):
                            volume_map_path = (
                                caseId + "/instate/" + wflowVars[v] + ".map"
                            )
                            if os.path.exists(volume_map_path):
                                volume_map = volume_map + pcr.readmap(volume_map_path)
                        # Convert from mm to m3
            ...
            for f in np.arange(0, len(flow_var)):
                ...
                if "0" in flow_csv:
                    del flow_csv["0"]
                if "# Timestep" in flow_csv:
                    del flow_csv["# Timestep"]
                ...
                if len(wflowVars) > 1:
                    for v in range(len(wflowVars) - 1):
                        ...
                        if "0" in flow_csv2:
                            del flow_csv2["0"]
                        if "# Timestep" in flow_csv2:
                            del flow_csv2["# Timestep"]
                ...
                if (
                    ...
                    level_mapstack = comp.mapstack[
                        (
                            comp.wflowVar == "KinWaveVolumeR+KinWaveVolumeL"
                            or comp.wflowVar == "KinWaveVolumeL+KinWaveVolumeR"
                        )
                    ].values
                    ...
                    if "0" in level_csv:
                        del level_csv["0"]
                    if "# Timestep" in level_csv:
                        del level_csv["# Timestep"]
                    if len(levVars) > 1:
                        for l in range(len(levVars) - 1):
                            level_path = (
                                caseId + "/" + runId + "/" + levVars[l + 1] + ".csv"
                            )
                            level_csv2 = pd.read_csv(level_path, sep=",", header=0)
                            if "0" in level_csv2:
                                del level_csv2["0"]
                            if "# Timestep" in level_csv2:
                                del level_csv2["# Timestep"]
                            level_csv = level_csv + level_csv2

                    DCL_sample = dw_pcrToDataBlock(
                        internalflowlength, gmap, Aggregation
                    )
                    ...
                    level_path = (
                        caseId + "/" + runId + "/" + level_mapstack[0] + "g.csv"
                    )
                    level_csv = pd.read_csv(level_path, sep=",", header=0)
                    if "0" in level_csv:
                        del level_csv["0"]
                ...
                if Emission:
                    ...
                    if bool(
                        np.isin(addtotflow_f[0], np.array(["T", "PT", "HT", "PHT"]))
                    ):
                        try:
                ...
                if Emission:
                    ...
                    if bool(
                        np.isin(addflowblock_f[0], np.array(["H", "PH", "HT", "PHT"]))
                    ):
                        addflow = True
                    else:
                        addflow = False
