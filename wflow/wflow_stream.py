...
class WflowModel(pcraster.framework.DynamicModel):
    ...
    def dynamic(self):
        ...
        *Required*
        ...
        self.available_water_below_cap = (
            self.Available_water * pcr.scalar(self.below_cap)
        ) + (pcr.max(0.0, Peff) * pcr.scalar(self.below_cap))

        # Soil wetting above capacity
        self.above_cap = (Aw_1 + Peff) > self.whc
        self.excess = (
            (Aw_1 * pcr.scalar(self.above_cap))
            + (Peff * pcr.scalar(self.above_cap))
            - (self.whc * pcr.scalar(self.above_cap))
        )
        self.available_water_above_cap = self.whc * pcr.scalar(self.above_cap)
        self.Available_water = (
            self.available_water_below_cap + self.available_water_above_cap
        )

        # soil drying
        self.drying = Peff <= 0.0
        self.exp_content = (Peff) / (self.whc)
        self.available_water_drying = (Aw_1 * pcr.scalar(self.drying)) * pcr.exp(
            self.exp_content
        )
