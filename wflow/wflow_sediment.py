...
class WflowModel(DynamicModel):  
  ...
  def smoothriv(self, river, subcatch, streamorder, param, wlgt):
      ...
      param = ifthenelse(streamorder == 9, windowaverage(order9, celllength()*wlgt) * windowtotal(unitmap, celllength()*wlgt) \
                         / windowtotal(riv9, celllength()*wlgt), param)
      param = ifthenelse(streamorder == 8, windowaverage(order8, celllength()*wlgt) * windowtotal(unitmap, celllength()*wlgt) \
                         / windowtotal(riv8, celllength()*wlgt), param)
      param = ifthenelse(streamorder == 7, windowaverage(order7, celllength()*wlgt) * windowtotal(unitmap, celllength()*wlgt) \
                         / windowtotal(riv7, celllength()*wlgt), param)
      param = ifthenelse(streamorder == 6, windowaverage(order6, celllength()*wlgt) * windowtotal(unitmap, celllength()*wlgt) \
                         / windowtotal(riv6, celllength()*wlgt), param)
      param = ifthenelse(streamorder == 5, windowaverage(order5, celllength()*wlgt) * windowtotal(unitmap, celllength()*wlgt) \
                         / windowtotal(riv5, celllength()*wlgt), param)
      param = ifthenelse(streamorder == 4, windowaverage(order4, celllength()*wlgt) * windowtotal(unitmap, celllength()*wlgt) \
                         / windowtotal(riv4, celllength()*wlgt), param)
      param = ifthenelse(streamorder == 3, windowaverage(order3, celllength()*wlgt) * windowtotal(unitmap, celllength()*wlgt) \
                         / windowtotal(riv3, celllength()*wlgt), param)
  ...
  def parameters(self):
      ...
      self.P_mapstack = self.Dir + configget(self.config, "inputmapstacks", "Precipitation", "/inmaps/P") #precipitation
      self.Int_mapstack = self.Dir + configget(self.config, "inputmapstacks", "Interception", "/inmaps/int") #rainfall interception
      self.SR_mapstack = self.Dir + configget(self.config, "inputmapstacks", "SurfaceRunoff", "/inmaps/run") #surface runoff
      self.WL_mapstack = self.Dir + configget(self.config, "inputmapstacks", "WaterLevel", "/inmaps/levKin") #water level
      ...
      modelparameters.append(self.ParamType(name="Interception",stack=self.Int_mapstack,type="timeseries",default=0.0,verbose=False,lookupmaps=[]))
      modelparameters.append(self.ParamType(name="SurfaceRunoff",stack=self.SR_mapstack,type="timeseries",default=0.0,verbose=True,lookupmaps=[]))
      modelparameters.append(self.ParamType(name="WaterLevel",stack=self.WL_mapstack,type="timeseries",default=0.0,verbose=False,lookupmaps=[]))
  ...
  def stateVariables(self):
      ...
      *Required*
      ...
      if self.RunRiverModel == 0:
          ...
          states = ['SedLoad', 'ClayLoad', 'SiltLoad',
                    'SandLoad', 'SaggLoad', 'LaggLoad',
                    'GravLoad', 'OutSedLoad', 'OutClayLoad',
                    'OutSiltLoad', 'OutSandLoad', 'OutSaggLoad',
                    'OutLaggLoad', 'OutGravLoad', 'RivStoreSed',
                    'RivStoreClay', 'RivStoreSilt', 'RivStoreSand',
                    'RivStoreSagg', 'RivStoreLagg', 'RivStoreGrav']
  ...
  def initial(self):
    ...
    *River*
    ...
    self.RainErodMethod = int(configget(self.config, "model", "rainerodmethod", "1"))
    self.LandTransportMethod = int(configget(self.config, "model", "landtransportmethod", "1"))
    self.RivTransportMethod = int(configget(self.config, "model", "rivtransportmethod", "1"))
    ...
    self.Altitude = self.wf_readmap(os.path.join(self.Dir,wflow_dem), 0.0, fail=True)
    ...
    self.TopoId = self.wf_readmap(os.path.join(self.Dir,wflow_subcatch), 0.0, fail=True)
    wflow_Hype = configget(self.config, "model", "wflow_Hype", "staticmaps/SUBID-HYPE-Rhine.map")
    self.HypeId = self.wf_readmap(os.path.join(self.Dir,wflow_Hype), 0.0, fail=False)
    wflow_ldd = configget(self.config, "model", "wflow_ldd", "staticmaps/wflow_ldd.map")
    self.TopoLdd = self.wf_readmap(os.path.join(self.Dir,wflow_ldd), 0.0, fail=True)
    wflow_river = configget(self.config, "model", "wflow_river", "staticmaps/wflow_river.map")
    self.River = self.wf_readmap(os.path.join(self.Dir,wflow_river), 0.0, fail=True)
    wflow_riverwidth = configget(self.config, "model", "wflow_riverwidth", "staticmaps/RiverWidth.map")
    self.RiverWidth = self.wf_readmap(os.path.join(self.Dir,wflow_riverwidth), 0.0, fail=True)
    ...
    self.DCL = self.wf_readmap(os.path.join(self.Dir,wflow_dcl), 0.0, fail=True) # Drain/River length
    ...
    #Soil
    wflow_clay = configget(self.config, "model", "wflow_clay", "staticmaps/percent_clay.map")
    self.PercentClay = self.wf_readmap(os.path.join(self.Dir,wflow_clay), 0.1, fail=True)
    wflow_silt = configget(self.config, "model", "wflow_silt", "staticmaps/percent_silt.map")
    self.PercentSilt = self.wf_readmap(os.path.join(self.Dir,wflow_silt), 0.1, fail=True)
    wflow_oc = configget(self.config, "model", "wflow_oc", "staticmaps/percent_oc.map")
    self.PercentOC = self.wf_readmap(os.path.join(self.Dir,wflow_oc), 0.1, fail=False)
...
#    self.BulkDensity = self.wf_readmap(os.path.join(self.Dir,wflow_bulk), 0.1, fail=False)
    ...
    self.PathFrac = self.readtblDefault(self.Dir + "/" + self.intbl + "/PathFrac.tbl", self.LandUse, self.TopoId, self.Soil, 0.01)
    
    #Soil model parameters
    self.ErosOv = self.readtblDefault(self.Dir + "/" + self.intbl + "/eros_ov.tbl", self.LandUse, self.TopoId, self.Soil, 0.90)
    
    if self.RunRiverModel == 1:    
        #River model parameters
        self.D50River = self.readtblFlexDefault(self.Dir + "/" + self.intbl + "/D50_River.tbl", 0.050, wflow_streamorder)
        self.D50River = ifthenelse(self.River == 1, self.D50River, scalar(0.0))
        self.CovRiver = self.readtblDefault(self.Dir + "/" + self.intbl + "/cov_River.tbl", self.LandUse, self.TopoId, self.Soil, 1.0)
    ...
    self.cellareaKm = (self.reallength/1000.)**2
    ...
    if self.slopecorr == 1:
        self.AltitudeMin = self.wf_readmap(os.path.join(self.Dir,"staticmaps/wflow_demmin.map"), 0.0, fail=True)
        #In 90m SRTM correct sea cells (-32768) with wflow_dem
        self.AltitudeMin = ifthenelse(self.AltitudeMin == -32768, scalar(0.0), self.AltitudeMin)
        ...
        self.RiverSlope = (upstream(self.TopoLdd, self.RiverDem)/self.nrupcell - downstream(self.TopoLdd, self.RiverDem)) \
                / (2 * self.reallength)
    ...
    #Smooth river slope
    self.RiverSlope = self.smoothriv(self.River, subcatch, self.streamorder, self.RiverSlope, 6)
...
#    self.RiverWidth = self.W
    ...
    if hasattr(self,"LAI"):
            ...
            self.CanopyGapFraction = self.readtblDefault(self.Dir + "/" + self.intbl + "/CanopyGapFraction.tbl", self.LandUse, self.TopoId, self.Soil, 0.1)
    ...
    self.ErosK = ifthenelse(pcrand(self.PercentClay>=40.,pcrand(self.PercentSand>=20.,self.PercentSand<=45.)),2.0,
    ifthenelse(pcrand(self.PercentClay>=27.,pcrand(self.PercentSand>=20.,self.PercentSand<=45.)),1.7,
    ifthenelse(pcrand(self.PercentSilt<=40.,self.PercentSand<=20.),2.0,
    ifthenelse(pcrand(self.PercentSilt>40.,self.PercentClay>=40.),1.6,
    ifthenelse(pcrand(self.PercentClay>=35.,self.PercentSand>=45.),1.9,
    ifthenelse(pcrand(self.PercentClay>=27.,self.PercentSand<20.),1.6,
    ifthenelse(pcrand(self.PercentClay<=10.,self.PercentSilt>=80.),1.2,
    ifthenelse(self.PercentSilt>=50,1.5,
    ifthenelse(pcrand(self.PercentClay>=7.,pcrand(self.PercentSand<=52.,self.PercentSilt>=28.)),2.0,
    ifthenelse(self.PercentClay>=20.,2.1,
    ifthenelse(self.PercentClay>=self.PercentSand-70.,2.6,
    ifthenelse(self.PercentClay>=(2.*self.PercentSand)-170.,3,scalar(1.9)))))))))))))
    
    # Compute USLE K factor
    if self.UsleKMethod == 1:
        self.UsleC =self.wf_readmap(os.path.join(self.Dir,"staticmaps/USLE_K.map"), 0.1, fail=True)
    if self.UsleKMethod == 2:
        # Calculate USLE K factor (from Renard et al. 1997, with the geometric mean particle diameter Dg)
        self.Dg = exp(0.01*(self.PercentClay*ln(0.001) + self.PercentSilt*ln(0.025) + self.PercentSand*ln(0.999))) #[mm]
        self.UsleK = 0.0034 + 0.0405 * exp(-1/2 * ((log10(self.Dg)+1.659)/0.7101)**2)
    ...
    if self.UsleKMethod == 3:
        ...
        self.UsleK = (0.2 + 0.3*exp(-0.0256*self.PercentSand * (1 - self.PercentSilt/100))) \
            * (self.PercentSilt/(max(0.01,self.PercentClay+self.PercentSilt)))**0.3 \
            * (1 - (0.25*self.PercentOC)/(self.PercentOC + exp(3.72 - 2.95*self.PercentOC))) \
    ...
    if self.UsleCMethod == 1:
        self.UsleC =self.wf_readmap(os.path.join(self.Dir,"staticmaps/USLE_C.map"), 0.1, fail=True)
    if self.UsleCMethod == 2:
        ...
        self.UsleC = self.readtblDefault(self.Dir + "/" + self.intbl + "/USLE_C.tbl", self.LandUse, self.TopoId, self.Soil, 1.0)


    #Parameters for either EUROSEM or ANSWERS rainfall erosion
    if self.RainErodMethod == 1: #EUROSEM
        #Canopy height [m]
        self.CanopyHeight =self.wf_readmap(os.path.join(self.Dir,"staticmaps/canopy_height.map"), 1.0, fail=True)
        #Coefficient
        self.ErosSpl = self.readtblDefault(self.Dir + "/" + self.intbl + "/eros_spl.tbl", self.LandUse, self.TopoId, self.Soil, 2.0)
    if self.RainErodMethod == 2: #ANSWERS
        #Coefficient
        self.ErosSpl = self.readtblDefault(self.Dir + "/" + self.intbl + "/eros_spl.tbl", self.LandUse, self.TopoId, self.Soil, 0.108)
    ...
    if (self.LandTransportMethod == 2 or self.LandTransportMethod == 3):
        ...
        self.PercentSand999 = self.PercentSand * (999-25)/(1000-25) #%sand with a mean radius of 999um instead of 1000
        self.vd50 = ln((1/((self.PercentClay+self.PercentSilt)/100) -1) / (1/(self.PercentClay/100) -1))
        self.wd50 = ln((1/((self.PercentClay+self.PercentSilt+self.PercentSand999)/100) -1) / (1/(self.PercentClay/100) -1))
        ...
        self.D50 = 1 + (-1/self.ud50 * ln(1 / (1/(self.PercentClay/100) -1)))**(1/self.cd50) #[um]
    ...
    #    self.percent_coars = self.percent_sand - self.percent_fvfs
        ...
        self.FracSagg = ifthenelse(self.PercentClay<25, 2.0 * self.PercentClay/100,
                                ifthenelse(self.PercentClay>50, 0.57, 
                                0.28*(self.PercentClay/100-0.25)+0.5))
        self.FracLagg = 1.0 - self.FracClay - self.FracSilt - self.FracSand - self.FracSagg
    ...
    if self.RunRiverModel == 1:
        ...
        self.FracClayRiv = ifthenelse(self.D50River <= 0.005, 0.65, ifthenelse(self.D50River > 2.0, 0.005, scalar(0.15)))
        self.FracSiltRiv = ifthenelse(pcrand(self.D50River > 0.005, self.D50River <= 0.050), 0.65, scalar(0.15))
        self.FracSandRiv = ifthenelse(pcrand(self.D50River > 0.050, self.D50River <= 2.0), 0.65, scalar(0.15))
        ...
        if self.RivTransportMethod == 2:
            self.cBagnold = self.readtblDefault(self.Dir + "/" + self.intbl + "/c_Bagnold.tbl", self.LandUse, self.TopoId, self.Soil, 0.0015)
            self.expBagnold = self.readtblDefault(self.Dir + "/" + self.intbl + "/exp_Bagnold.tbl", self.LandUse, self.TopoId, self.Soil, 1.4)
        
        #Parameters of Kodatie transport formula
        if self.RivTransportMethod == 3:
            self.aK = ifthenelse(self.D50River <= 0.05, 281.4, ifthenelse(self.D50River <= 0.25, 2829.6,
                            ifthenelse(self.D50River <= 2, 2123.4, scalar(431884.8))))
            self.bK = ifthenelse(self.D50River <= 0.05, 2.622, ifthenelse(self.D50River <= 0.25, 3.646,
                            ifthenelse(self.D50River <= 2, 3.300, scalar(1.0))))
            self.cK = ifthenelse(self.D50River <= 0.05, 0.182, ifthenelse(self.D50River <= 0.25, 0.406,
                            ifthenelse(self.D50River <= 2, 0.468, scalar(1.0))))
            self.dK = ifthenelse(self.D50River <= 0.05, 0.0, ifthenelse(self.D50River <= 0.25, 0.412,
        ...
        self.TCrBank = (0.1 + 0.1779*(100*self.FracClayRiv+100*self.FracSiltRiv)+0.0028*(100*self.FracClayRiv+100*self.FracSiltRiv)**2 \
                        -2.34*10**(-5)*(100*self.FracClayRiv+100*self.FracSiltRiv)**3)*self.CovRiver
        self.KdBank = 0.2 * self.TCrBank ** (-0.5) * 10**(-6)
        #Bed from Shields diagram
        E = ((2.65-1)*9.81*(self.D50River*10**(-3))**3/(1*10**(-12)))**(0.33)
        self.TCrBed = (2.65-1)*9.81*self.D50River*(0.13*E**(-0.392)*exp(-0.015*E**2) + 0.045*(1-exp(-0.068*E)))
    ...
    if hasattr(self, "ReserVoirSimpleLocs"):
        ...
        self.filter_Eros_TC = ifthenelse(
            boolean(cover(res_area, scalar(0.0))), res_area * 0.0, self.filter_Eros_TC
        )
    else:
        self.nrresSimple = 0    
    ...
    if (self.nrresSimple + self.nrresComplex) > 0:
        ...
        self.TopoLddOrg = lddrepair(cover(ifthen(boolean(self.ReserVoirLocs), ldd(5)), self.TopoLdd))
  ...
  def default_summarymaps(self):
      ...
      *Optional*
      ...
      """
      lst = ['self.PercentSand', 'self.UsleK',
             'self.UsleC', 'self.ErodK', 
             'self.D50River','self.FracClayRiv',
             'self.filter_Eros_TC', 'self.RiverSlope',
             'self.RiverWidth', 'self.RiverDem',
             'self.UpArea', 'self.cellareaKm']
  ...
  def dynamic(self):
      ...
      *River model*
      ...
      if self.RainErodMethod == 1:
          ...
          self.KeDirect = max(8.95 + 8.44 * log10(max(0.0001,self.rintnsty)), 0.0)  #variant, most used in distributed models
          ...
          rDepthDirect = max(rDepthTot - rDepthLeaf - self.Interception, 0.0) #throughfall
      
          #Total kinetic energy by rainfall [J/m^2]
          self.KeTotal = (rDepthDirect * self.KeDirect + rDepthLeaf * self.KeLeaf) *0.001
      
          # Rainfall/Splash erosion
          self.SedSpl = self.ErosK * self.KeTotal * exp(-self.ErosSpl * self.WaterLevel) # [g/m^2]
          self.Sedspl = self.cellareaKm * self.SedSpl #* self.timestepsecs # [ton/cell/timestep]
      ...
      if self.RainErodMethod == 2:
          ...
          # Splash erosion
          self.SedSpl = 0.108 * self.UsleC * self.UsleK * self.cellareaKm * 10**6 * self.rintnsty**2 # [kg/min]
          self.SedSpl = self.timestepsecs/60.0 * 10**(-3) * self.SedSpl # [ton/timestep]
      ...
      self.OvRun = cover(ifthenelse(self.River == 1, 0.0, self.SurfaceRunoff), self.SurfaceRunoff) #[m3/s]
      ...
      self.SedOv = self.ErosOv * self.UsleC * self.UsleK * self.cellareaKm * 10**6 * self.sinSlope * self.OvRunRate # [kg/min]
      self.SedOv = self.timestepsecs/60.0 * 10**(-3) * self.SedOv # [ton/timestep]
    ...
    # If just the soil loss model is run, use Govers equation without particle differentiation
      ...
      if (self.LandTransportMethod == 2 or self.LandTransportMethod == 3):
          if self.LandTransportMethod == 2:
              #Unit stream power
              self.velocity = cover(ifthenelse(self.WaterLevel > 0, self.OvRun / (self.reallength * self.WaterLevel), 0.0), 0.0) #[m/s]
              ...
              self.TCf = ifthenelse(self.omega > 0.4, self.cGovers * (self.omega - 0.4)**self.nGovers * 2650, 0.0) #[kg/m3]
              #self.TC = self.TCf / (1 - self.TCf/2650) #[kg/m3]
              #self.TC = max(self.TC, 2650)
              self.TC = self.TCf * self.OvRun * self.timestepsecs * 10**(-3) #[ton/cell/timestep]     
              # Remove nodata values
              ...
              if (self.nrresSimple + self.nrresComplex) > 0 and self.filterResArea == 0:
                    self.TC = ifthenelse(pcrand(self.filter_Eros_TC == 0, self.River == 0), 10**9, self.TC)
          
          elif self.LandTransportMethod == 3:
              #Transport capacity from Yalin
              self.OvLevel = cover(ifthenelse(self.River == 1, 0.0, self.WaterLevel), self.WaterLevel) #[m]
              self.delta = max(self.OvLevel * self.sinSlope / (self.D50*10**(-3)*(2650/1000-1)) /0.06 -1, 0.0)
              self.TC = self.reallength/self.OvRun * (2650-1000) * self.D50*10**(-3) * (9.81*self.OvLevel*self.sinSlope) * \
                  0.635 * self.delta * (1 - ln(1 + self.delta*2.45/(2650/1000)**0.4*0.06**0.5)/self.delta*2.45/(2650/1000)**0.4*0.06**0.5) #[kg/m3]
              self.TC = ifthen(self.subcatch > 0, cover(self.TC*self.OvRun*self.timestepsecs*10**(-3), 0.0)) #[ton/cell/timestep]
              #Assume that eroded soil on lake cells all reach the river cells of the reservoir
              if (self.nrresSimple + self.nrresComplex) > 0 and self.filterResArea == 0:
                    self.TC = ifthenelse(pcrand(self.filter_Eros_TC == 0, self.River == 0), 10**9, self.TC)
          ...
          self.SedFlux = accucapacityflux(self.TopoLdd, self.SoilLoss, self.TCRiv) #[ton/cell/tinestep]      
          ...
          self.HYPEOvSedCatch = areatotal(self.OvSed, self.HYPEcatch) #[ton/cell/timestep]
          self.HYPEOvSedCatch = self.HYPEOvSedCatch * 1000 / (self.cellareaKm *100) #[kg/ha/timestep]
          ...
          self.DClay = max(self.OvLevel * self.sinSlope / (2*10**(-6)*(2650/1000-1)) /0.06 -1, 0.0)
          self.DSilt = max(self.OvLevel * self.sinSlope / (10*10**(-6)*(2650/1000-1)) /0.06 -1, 0.0)
          self.DSand = max(self.OvLevel * self.sinSlope / (200*10**(-6)*(2650/1000-1)) /0.06 -1, 0.0)
          self.DSagg = max(self.OvLevel * self.sinSlope / (30*10**(-6)*(2650/1000-1)) /0.06 -1, 0.0)
          self.DLagg = max(self.OvLevel * self.sinSlope / (500*10**(-6)*(2650/1000-1)) /0.06 -1, 0.0)
          ...
          self.TCClay = self.reallength/self.OvRun * (2650-1000) * 2*10**(-6) * (9.81*self.OvLevel*self.sinSlope) *  self.DClay/self.Dtot * \
              0.635 * self.DClay * (1 - ln(1 + self.DClay*2.45/(2650/1000)**0.4*0.06**0.5)/self.DClay*2.45/(2650/1000)**0.4*0.06**0.5) #[kg/m3]
          self.TCClay = ifthen(self.subcatch > 0, cover(self.TCClay*self.OvRun*self.timestepsecs*10**(-3), 0.0)) #[ton/cell/timestep]
    
          self.TCSilt = self.reallength/self.OvRun * (2650-1000) * 10*10**(-6) * (9.81*self.OvLevel*self.sinSlope) *  self.DSilt/self.Dtot * \
              0.635 * self.DSilt * (1 - ln(1 + self.DSilt*2.45/(2650/1000)**0.4*0.06**0.5)/self.DSilt*2.45/(2650/1000)**0.4*0.06**0.5) #[kg/m3]
          self.TCSilt = ifthen(self.subcatch > 0, cover(self.TCSilt*self.OvRun*self.timestepsecs*10**(-3), 0.0)) #[ton/cell/timestep]    
          
          self.TCSand = self.reallength/self.OvRun * (2650-1000) * 200*10**(-6) * (9.81*self.OvLevel*self.sinSlope) *  self.DSand/self.Dtot * \
              0.635 * self.DSand * (1 - ln(1 + self.DSand*2.45/(2650/1000)**0.4*0.06**0.5)/self.DSand*2.45/(2650/1000)**0.4*0.06**0.5) #[kg/m3]
          self.TCSand = ifthen(self.subcatch > 0, cover(self.TCSand*self.OvRun*self.timestepsecs*10**(-3), 0.0)) #[ton/cell/timestep]
          
          self.TCSagg = self.reallength/self.OvRun * (2650-1000) * 30*10**(-6) * (9.81*self.OvLevel*self.sinSlope) *  self.DSagg/self.Dtot * \
              0.635 * self.DSagg * (1 - ln(1 + self.DSagg*2.45/(2650/1000)**0.4*0.06**0.5)/self.DSagg*2.45/(2650/1000)**0.4*0.06**0.5) #[kg/m3]
          self.TCSagg = ifthen(self.subcatch > 0, cover(self.TCSagg*self.OvRun*self.timestepsecs*10**(-3), 0.0)) #[ton/cell/timestep]
          
          self.TCLagg = self.reallength/self.OvRun * (2650-1000) * 500*10**(-6) * (9.81*self.OvLevel*self.sinSlope) *  self.DLagg/self.Dtot * \
              0.635 * self.DLagg * (1 - ln(1 + self.DLagg*2.45/(2650/1000)**0.4*0.06**0.5)/self.DLagg*2.45/(2650/1000)**0.4*0.06**0.5) #[kg/m3]
          self.TCLagg = ifthen(self.subcatch > 0, cover(self.TCLagg*self.OvRun*self.timestepsecs*10**(-3), 0.0)) #[ton/cell/timestep]
          
          #Assume that eroded soil on lake cells all reach the river cells of the reservoir
          if (self.nrresSimple + self.nrresComplex) > 0:
                self.TCClay = cover(ifthenelse(self.filter_Eros_TC == 0, 10**9, self.TCClay),self.TCClay)
                self.TCClay = cover(ifthenelse(self.River == 1, 0.0, self.TCClay),self.TCClay)
                self.TCSilt = cover(ifthenelse(self.filter_Eros_TC == 0, 10**9, self.TCSilt),self.TCSilt)
                self.TCSilt = cover(ifthenelse(self.River == 1, 0.0, self.TCSilt),self.TCSilt)
                self.TCSand = cover(ifthenelse(self.filter_Eros_TC == 0, 10**9, self.TCSand),self.TCSand)
                self.TCSand = cover(ifthenelse(self.River == 1, 0.0, self.TCSand),self.TCSand)
                self.TCSagg = cover(ifthenelse(self.filter_Eros_TC == 0, 10**9, self.TCSagg),self.TCSagg)
                self.TCSagg = cover(ifthenelse(self.River == 1, 0.0, self.TCSagg),self.TCSagg)
                self.TCLagg = cover(ifthenelse(self.filter_Eros_TC == 0, 10**9, self.TCLagg),self.TCLagg)
                self.TCLagg = cover(ifthenelse(self.River == 1, 0.0, self.TCLagg),self.TCLagg)
          
          #Eroded sediment in overland flow reaching the river system per particle class [ton/cell/timestep]
          self.InLandClay = cover(ifthenelse(self.River == 1, accucapacitystate(self.TopoLdd, self.LandErodClay, self.TCClay), 0.0), 0.0)
          self.InLandSilt = cover(ifthenelse(self.River == 1, accucapacitystate(self.TopoLdd, self.LandErodSilt, self.TCSilt), 0.0), 0.0)      
          self.InLandSand = cover(ifthenelse(self.River == 1, accucapacitystate(self.TopoLdd, self.LandErodSand, self.TCSand), 0.0), 0.0)      
          self.InLandSagg = cover(ifthenelse(self.River == 1, accucapacitystate(self.TopoLdd, self.LandErodSagg, self.TCSagg), 0.0), 0.0)
          self.InLandLagg = cover(ifthenelse(self.River == 1, accucapacitystate(self.TopoLdd, self.LandErodLagg, self.TCLagg), 0.0), 0.0)
          self.InLandSed = self.InLandClay + self.InLandSilt + self.InLandSand + self.InLandSagg + self.InLandLagg #total
          
          #Sediment input reaching the river system per particle class and per HYPE subcatchment [kg/ha/timestep]
          self.ClayCatch = areatotal(self.InLandClay, self.HYPEcatch) * 1000 / (self.cellareaKm *100)
          self.SiltCatch = areatotal(self.InLandSilt, self.HYPEcatch) * 1000 / (self.cellareaKm *100) 
          self.SandCatch = areatotal(self.InLandSand, self.HYPEcatch) * 1000 / (self.cellareaKm *100)
          self.SaggCatch = areatotal(self.InLandSagg, self.HYPEcatch) * 1000 / (self.cellareaKm *100)
          self.LaggCatch = areatotal(self.InLandLagg, self.HYPEcatch) * 1000 / (self.cellareaKm *100)
          self.HYPESedCatch = self.ClayCatch + self.SiltCatch + self.SandCatch + self.SaggCatch + self.LaggCatch #total
      ...
      if self.RunRiverModel == 1:
          ...
          self.InSedLoad = self.OldSedLoad + upstream(self.TopoLdd, self.OutSedLoad) + self.InLandSed
          self.InClayLoad = self.OldClayLoad + upstream(self.TopoLdd, self.OutClayLoad) + self.InLandClay
          self.InSiltLoad = self.OldSiltLoad + upstream(self.TopoLdd, self.OutSiltLoad) + self.InLandSilt
          self.InSandLoad = self.OldSandLoad + upstream(self.TopoLdd, self.OutSandLoad) + self.InLandSand
          self.InSaggLoad = self.OldSaggLoad + upstream(self.TopoLdd, self.OutSaggLoad) + self.InLandSagg
          self.InLaggLoad = self.OldLaggLoad + upstream(self.TopoLdd, self.OutLaggLoad) + self.InLandLagg
          self.InGravLoad = self.OldGravLoad + upstream(self.TopoLdd, self.OutGravLoad)
                
          ''' River erosion '''
          #Hydraulic radius of the river [m] (rectangular channel)
          self.HydRad = self.WaterLevel*self.RiverWidth / (self.RiverWidth + 2*self.WaterLevel)
          
          #Transport capacity 
          #Engelund and Hansen transport formula
          if self.RivTransportMethod == 1 :
              #Shields parameter
              self.ThetaShields = 1000 * 9.81 * self.HydRad * self.RiverSlope / ((2650-1000) * 9.81 * self.D50River/1000)
              self.vmean = ifthenelse(pcrand(self.WaterLevel > 0, self.River ==1), self.SurfaceRunoff/(self.RiverWidth*self.WaterLevel), scalar(0.0))
    ...
    #              / ((2650-1000)/1000*9.81*self.D50River/1000)**(0.5) * self.ThetaShields**(0.5) # sediment concentration by weight
              self.Cw = min( 1.0, ifthenelse(pcrand(self.HydRad > 0, self.River ==1), 2.65 * 0.05 * self.vmean * self.vshear**3 \
                      / ((2.65-1)**2 * 9.81**2 * self.D50River * self.HydRad), scalar(0.0))) #concentration by weight
              self.MaxSedLoad =  max(0.0, self.Cw / (self.Cw + (1-self.Cw)*2.65) * 2.65) #[tons/m3]
              self.MaxSedLoad = self.MaxSedLoad * (self.WaterLevel * self.RiverWidth * self.DCL + self.SurfaceRunoff*self.timestepsecs) #[ton]
          
          #Simplified Bagnold transport formula
          if self.RivTransportMethod == 2 :
              self.MaxSedLoad = self.cBagnold * (self.SurfaceRunoff / (self.WaterLevel * self.RiverWidth))**self.expBagnold #[ton/m3]
              self.MaxSedLoad = self.MaxSedLoad * (self.WaterLevel * self.RiverWidth * self.DCL + self.SurfaceRunoff*self.timestepsecs) #[ton]
    
          #Kodatie transport formula
          if self.RivTransportMethod == 3 :
              self.vmean = ifthenelse(pcrand(self.WaterLevel > 0, self.River ==1), self.SurfaceRunoff/(self.RiverWidth*self.WaterLevel), scalar(0.0))
              self.MaxSedLoad = (self.aK * self.vmean**(self.bK) * self.WaterLevel**(self.cK) * self.RiverSlope**(self.dK)) * (self.RiverWidth) #[tons]
          ...
          if self.RivTransportMethod == 4 :
              ...
              self.vcr = ifthenelse(self.var1 >= 70, 2.05 * self.wsRiv, self.wsRiv *(2.5 / (log10(self.var1)-0.06) + 0.66))
              
              #Sand equation
              self.logCppm = 5.435 - 0.286*log10(self.var2) - 0.457*log10(self.vshear/self.wsRiv) + \
                  (1.799 - 0.409*log10(self.var2) - 0.314*log10(self.vshear/self.wsRiv)) * \
                  log10((self.SurfaceRunoff/(self.RiverWidth*self.WaterLevel) - self.vcr) * self.RiverSlope/self.wsRiv)
              #Gravel equation
              self.logCppm = ifthenelse(self.D50River < 2.0, self.logCppm, 6.681 - 0.633*log10(self.var2) - 4.816*log10(self.vshear/self.wsRiv) + \
                  (2.784 - 0.305*log10(self.var2) - 0.282*log10(self.vshear/self.wsRiv)) * \
                  log10((self.SurfaceRunoff/(self.RiverWidth*self.WaterLevel) - self.vcr) * self.RiverSlope/self.wsRiv))
          ...
          if self.RivTransportMethod == 5 :
              self.wsRiv = 411 * self.D50River**2 / 3600
              self.psi = ((2.65-1)*9.81*self.WaterLevel*self.wsRiv*(log10(self.WaterLevel/self.D50River))**2)**(0.5)
              self.Cw = 1430 * (0.86 + (self.psi)**(0.5)) * (self.psi)**(1.5) / (0.016 + self.psi) * 10**(-6) #weight          
              self.MaxSedLoad = max(0.0, self.Cw / (self.Cw + (1-self.Cw)*2.65) * 2.65) #[tons/m3]
              self.MaxSedLoad = self.MaxSedLoad * (self.WaterLevel * self.RiverWidth * self.DCL + self.SurfaceRunoff*self.timestepsecs) #[ton]
          ...
          self.SFBank = ifthenelse(pcrand(self.WaterLevel > 0, self.River == 1), exp(-3.230 * log10(self.RiverWidth / self.WaterLevel + 3) + 6.146), scalar(0.0)) #[%]
          #Effective shear stress on river bed and banks [N/m2]
          self.TEffBank = ifthenelse(pcrand(self.WaterLevel > 0, self.River == 1), 1000 * 9.81 * self.HydRad * self.RiverSlope * self.SFBank/100 \
                                     * (1 + self.RiverWidth / (2*self.WaterLevel)), scalar(0.0))
          self.TEffBed = 1000 * 9.81 * self.HydRad * self.RiverSlope * (1 - self.SFBank/100) * (1 + 2*self.WaterLevel/self.RiverWidth)
          #Potential erosion rates of the bed and bank [t/cell/timestep] (assuming only one bank is eroding)
          self.ERBank = max(0.0, self.KdBank * (self.TEffBank - self.TCrBank) * (self.DCL * self.WaterLevel) * 1.4 * self.timestepsecs) #1.4 is bank default bulk density
          self.ERBed = max(0.0, self.KdBed * (self.TEffBed - self.TCrBed) * (self.DCL * self.RiverWidth) * 1.5 * self.timestepsecs) #1.5 is bed default bulk density
          #Relative potential erosion rates of the bed and bank [-]
          self.RTEBank = ifthenelse(self.ERBank + self.ERBed > 0.0, self.ERBank/(self.ERBank+self.ERBed), 0.0)
          ...
          self.BankSedLoad = ifthenelse(self.EffSedEx == 0, 0.0, ifthenelse(self.EffSedEx*self.RTEBank <= self.ERBank, 
          ...
          self.BedSedLoad = ifthenelse(self.EffSedEx == 0, 0.0, ifthenelse(self.EffSedEx*self.RTEBed <= self.ERBed,
          ...
          self.DegStoreClay = ifthenelse(self.RivStoreClay >= self.SedEx, self.SedEx, self.RivStoreClay)
          self.RivStoreClay = self.RivStoreClay - self.DegStoreClay #update store
          self.SedEx = max(self.SedEx - self.DegStoreClay, 0.0) #update amount of sediment that need to be degraded
          
          self.DegStoreSilt = ifthenelse(self.RivStoreSilt >= self.SedEx, self.SedEx, self.RivStoreSilt)
          self.RivStoreSilt = self.RivStoreSilt - self.DegStoreSilt
          self.SedEx = max(self.SedEx - self.DegStoreSilt, 0.0)
          
          self.DegStoreSagg = ifthenelse(self.RivStoreSagg >= self.SedEx, self.SedEx, self.RivStoreSagg)
          self.RivStoreSagg = self.RivStoreSagg - self.DegStoreSagg
          self.SedEx = max(self.SedEx - self.DegStoreSagg, 0.0)
          
          self.DegStoreSand = ifthenelse(self.RivStoreSand >= self.SedEx, self.SedEx, self.RivStoreSand)
          self.RivStoreSand = self.RivStoreSand - self.DegStoreSand
          self.SedEx = max(self.SedEx - self.DegStoreSand, 0.0)
          
          self.DegStoreLagg = ifthenelse(self.RivStoreLagg >= self.SedEx, self.SedEx, self.RivStoreLagg)
          self.RivStoreLagg = self.RivStoreLagg - self.DegStoreLagg
          self.SedEx = max(self.SedEx - self.DegStoreLagg, 0.0)
          
          self.DegStoreGrav = ifthenelse(self.RivStoreGrav >= self.SedEx, self.SedEx, self.RivStoreGrav)
          self.RivStoreGrav = self.RivStoreGrav - self.DegStoreGrav
          self.SedEx = max(self.SedEx - self.DegStoreGrav, 0.0)
          
          self.DegStoreSed = self.DegStoreClay + self.DegStoreSilt + self.DegStoreSand + self.DegStoreSagg + self.DegStoreLagg + self.DegStoreGrav
          ...
          self.FracDepEx = ifthenelse(self.TCEx < 0, 1 - self.MaxSedLoad / self.InSedLoad, scalar(0.0))
          
          #Fractions of deposited particles in river cells from the Einstein formula [-]
          #Particle fall velocity [m/s] from Stokes
          self.x = ifthenelse(pcrand(self.SurfaceRunoff > 0, self.River ==1), 1.055*self.DCL / (self.SurfaceRunoff / self.RiverWidth), scalar(0.0))
          self.x = ifthen(self.subcatch > 0, cover(self.x, 0.0))
          
          self.FracDepClay = ifthenelse(self.TCEx >= 0, min( 1.0, 1 - 1 / exp(self.x * (411 * 0.002**2 / 3600))), self.FracDepEx)
          self.FracDepSilt = ifthenelse(self.TCEx >= 0, min( 1.0, 1 - 1 / exp(self.x * (411 * 0.010**2 / 3600))), self.FracDepEx)
          self.FracDepSand = ifthenelse(self.TCEx >= 0, min( 1.0, 1 - 1 / exp(self.x * (411 * 0.200**2 / 3600))), self.FracDepEx)
          self.FracDepSagg = ifthenelse(self.TCEx >= 0, min( 1.0, 1 - 1 / exp(self.x * (411 * 0.030**2 / 3600))), self.FracDepEx)
          self.FracDepLagg = ifthenelse(self.TCEx >= 0, min( 1.0, 1 - 1 / exp(self.x * (411 * 0.500**2 / 3600))), self.FracDepEx)
          self.FracDepGrav = ifthenelse(self.TCEx >= 0, min( 1.0, 1 - 1 / exp(self.x * (411 * 2.000**2 / 3600))), self.FracDepEx)  
          ...
          self.DepSedLoad = self.DepClay + self.DepSilt + self.DepSand + self.DepSagg + self.DepLagg + self.DepGrav
          ...
          if (self.nrresSimple + self.nrresComplex) > 0 and self.filterResArea == 0:
             self.VcRes = ifthenelse(self.ReserVoirLocs > 0, self.SurfaceRunoff / self.ResArea, 0.0)
             self.DepClay = cover(ifthen(self.ReserVoirLocs > 0, self.InClayLoad * min(1.0, (411/3600*0.002**2)/self.VcRes)), self.DepClay)
             self.DepSilt = cover(ifthen(self.ReserVoirLocs > 0, self.InSiltLoad * min(1.0, (411/3600*0.010**2)/self.VcRes)), self.DepSilt)
             self.DepSand = cover(ifthen(self.ReserVoirLocs > 0, self.InSandLoad * min(1.0, (411/3600*0.200**2)/self.VcRes)), self.DepSand)
             self.DepSagg = cover(ifthen(self.ReserVoirLocs > 0, self.InSaggLoad * min(1.0, (411/3600*0.030**2)/self.VcRes)), self.DepSagg)
             self.DepLagg = cover(ifthen(self.ReserVoirLocs > 0, self.InLaggLoad * min(1.0, (411/3600*0.500**2)/self.VcRes)), self.DepLagg)
             self.DepGrav = cover(ifthen(self.ReserVoirLocs > 0, self.InGravLoad * min(1.0, (411/3600*2.000**2)/self.VcRes)), self.DepGrav)
             self.DepSedLoad = cover(ifthen(self.ReserVoirLocs > 0, self.DepClay + self.DepSilt + self.DepSand + self.DepSagg + self.DepLagg + self.DepGrav), self.DepSedLoad)   
          ...
          self.OutFracWat = min(self.SurfaceRunoff*self.timestepsecs / (self.WaterLevel*self.RiverWidth*self.DCL + self.SurfaceRunoff*self.timestepsecs), 0.999999)
          
          self.OutSedLoad = ifthen(self.subcatch > 0, cover(max(0.0, (self.InSedLoad+self.RivErodSed-self.DepSedLoad) * self.OutFracWat), 0.0))
          self.OutClayLoad = ifthen(self.subcatch > 0, cover(max(0.0, (self.InClayLoad+self.RivErodClay-self.DepClay) * self.OutFracWat), 0.0))
          self.OutSiltLoad = ifthen(self.subcatch > 0, cover(max(0.0, (self.InSiltLoad+self.RivErodSilt-self.DepSilt) * self.OutFracWat), 0.0))
          self.OutSandLoad = ifthen(self.subcatch > 0, cover(max(0.0, (self.InSandLoad+self.RivErodSand-self.DepSand) * self.OutFracWat), 0.0))
          self.OutSaggLoad = ifthen(self.subcatch > 0, cover(max(0.0, (self.InSaggLoad+self.RivErodSagg-self.DepSagg) * self.OutFracWat), 0.0))
          self.OutLaggLoad = ifthen(self.subcatch > 0, cover(max(0.0, (self.InLaggLoad+self.RivErodLagg-self.DepLagg) * self.OutFracWat), 0.0))
          self.OutGravLoad = ifthen(self.subcatch > 0, cover(max(0.0, (self.InGravLoad+self.RivErodGrav-self.DepGrav) * self.OutFracWat), 0.0))
          
          
          ''' Mass balance, sediment concentration in each river cell '''
          #Sediment load [ton/cell/timestep]
          #0 in case all sediment are deposited in the cell
          self.SedLoad = max(0.0, self.InSedLoad + self.RivErodSed - self.DepSedLoad - self.OutSedLoad)
          self.ClayLoad = max(0.0, self.InClayLoad + self.RivErodClay - self.DepClay - self.OutClayLoad)
          self.SiltLoad = max(0.0, self.InSiltLoad + self.RivErodSilt - self.DepSilt - self.OutSiltLoad)
          self.SandLoad = max(0.0, self.InSandLoad + self.RivErodSand - self.DepSand - self.OutSandLoad)
          self.SaggLoad = max(0.0, self.InSaggLoad + self.RivErodSagg - self.DepSagg - self.OutSaggLoad)
          self.LaggLoad = max(0.0, self.InLaggLoad + self.RivErodLagg - self.DepLagg - self.OutLaggLoad)
          self.GravLoad = max(0.0, self.InGravLoad + self.RivErodGrav - self.DepGrav - self.OutGravLoad)
          ...
          self.BedConc = (self.OutSandLoad + self.OutSaggLoad + self.OutLaggLoad + self.OutGravLoad) * self.ToConc
      ...
      self.SpecRun = self.SurfaceRunoff * 1000 / accuflux(self.TopoLdd, self.cellareaKm) #[L/km2/s]
